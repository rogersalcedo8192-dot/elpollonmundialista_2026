...
export function LigaMillonariosView({ currentUser, getHeaders, activeSection, onNavigate }: Props) {
  ...
  const footballDates: string[] = Array.from(new Set<string>(matches.map((match) => match.stage)));
    ...
    <section className="min-w-0 max-w-full space-y-5 overflow-x-hidden">
      ...
      <div className={`${["pronosticos", "favoritos", "ranking", "notificaciones", "admin"].includes(activeSection) ? "grid" : "hidden"} gap-5 xl:grid-cols-[1.6fr_1fr]`}>
        <div id="liga-pronosticos" ref={predictionsTopRef} className={activeSection === "pronosticos" ? "space-y-3" : "hidden"}>
          {showBackToTop && <button type="button" onClick={() => predictionsTopRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })} className="fixed bottom-20 left-1/2 z-40 inline-flex -translate-x-1/2 items-center gap-1.5 rounded-full border border-blue-200/80 bg-white/95 px-4 py-2.5 text-[11px] font-black text-blue-700 shadow-xl backdrop-blur hover:bg-blue-50 sm:bottom-auto sm:left-auto sm:right-3 sm:top-1/2 sm:-translate-x-0 sm:-translate-y-1/2 sm:px-3 sm:py-2 dark:border-blue-800 dark:bg-slate-950/95 dark:text-blue-300" title="Volver al inicio del calendario" aria-label="Volver al inicio del calendario"><ChevronUp className="h-4 w-4 stroke-[3]" /><span>Arriba</span></button>}
          <div className="mb-4 flex flex-col gap-3 border-b border-slate-100 pb-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between dark:border-slate-800"><div><h2 className="flex items-center gap-2 text-lg font-bold sm:text-xl"><Calendar className="h-5 w-5 text-blue-600" /> Calendario & Pronósticos</h2><p className="mt-1 text-xs text-slate-500">Introduce marcadores. El registro cierra cinco minutos antes de cada partido.</p></div><div className="grid grid-cols-2 gap-2 rounded-xl bg-slate-100 px-3 py-2 font-mono text-[11px] text-slate-700 sm:flex sm:gap-4 sm:py-1.5 sm:text-xs dark:bg-slate-800 dark:text-slate-300"><span>Registrados: <b>{registeredPredictions}</b></span><span>Pendientes: <b>{pendingPredictions}</b></span></div></div>
          <div className="min-w-0 max-w-full space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs dark:border-slate-800 dark:bg-slate-900/60"><div className="flex items-center justify-between gap-3"><button type="button" onClick={() => setMatchFiltersOpen((current) => !current)} className={`flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl px-3 font-black sm:flex-none ${matchFiltersOpen || hasMatchFilters ? "bg-blue-700 text-white" : "border border-slate-200 bg-white text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}>{matchFiltersOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />} Filtros {hasMatchFilters && <span className="rounded-full bg-white/20 px-1.5 py-0.5 text-[9px]">activos</span>}</button><span className="shrink-0 text-[10px] font-semibold text-slate-500 sm:text-[11px]">{filteredMatches.length}/{matches.length} partidos</span></div><div className="grid grid-cols-4 gap-1.5 sm:hidden" aria-label="Acceso rápido por fecha"><button type="button" onClick={() => setRoundFilter("Todas")} className={`min-h-10 min-w-0 rounded-lg px-1 text-[9px] font-black ${roundFilter === "Todas" ? "bg-blue-700 text-white" : "border border-slate-200 bg-white text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"}`}>Todas</button>{footballDates.map((stage) => <button type="button" key={stage} onClick={() => setRoundFilter(stage)} className={`min-h-10 min-w-0 truncate rounded-lg px-1 text-[9px] font-black ${roundFilter === stage ? "bg-blue-700 text-white" : "border border-slate-200 bg-white text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"}`}>{stage.replace(/Jornada|Fecha/i, "F.")}</button>)}</div>{matchFiltersOpen && <div className="grid min-w-0 gap-3 sm:grid-cols-3"><label className="hidden font-bold text-slate-600 sm:block dark:text-slate-300">Fecha futbolera<select value={roundFilter} onChange={(event) => setRoundFilter(event.target.value)} className="mt-1 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm dark:border-slate-700 dark:bg-slate-800"><option value="Todas">Todas las fechas</option>{footballDates.map((stage) => { const match = matches.find((candidate) => candidate.stage === stage); return <option key={stage} value={stage}>{stage}{match ? ` · ${new Date(match.date).toLocaleDateString("es-CO", { timeZone: "America/Bogota", day: "numeric", month: "short" })}` : ""}</option>; })}</select></label><label className="min-w-0 font-bold text-slate-600 dark:text-slate-300">Estado<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)} className="mt-1 min-h-12 w-full max-w-full rounded-xl border border-slate-200 bg-white px-3 text-base sm:text-sm dark:border-slate-700 dark:bg-slate-800"><option value="all">Ver todos</option><option value="pending">Abiertos</option><option value="finished">Finalizados</option></select></label><label className="min-w-0 font-bold text-slate-600 dark:text-slate-300">Buscar rival o estadio<input type="search" value={matchSearch} onChange={(event) => setMatchSearch(event.target.value)} placeholder="Bucaramanga, El Campín..." className="mt-1 min-h-12 w-full max-w-full rounded-xl border border-slate-200 bg-white px-3 text-base sm:text-sm dark:border-slate-700 dark:bg-slate-800" /></label></div>}<div className="flex items-center justify-between gap-3"><span className="hidden text-[11px] text-slate-500 sm:inline">Filtra por jornada oficial, estado o rival.</span>{hasMatchFilters && <button type="button" onClick={() => { setRoundFilter("Todas"); setStatusFilter("all"); setMatchSearch(""); }} className="ml-auto min-h-10 rounded-xl bg-slate-200 px-3 font-black text-slate-700 dark:bg-slate-800 dark:text-slate-200">Limpiar filtros</button>}</div></div>
