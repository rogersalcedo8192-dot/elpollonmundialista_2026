    ...
    <section className="space-y-5">
      ...
      <div className={`${["pronosticos", "favoritos", "ranking", "notificaciones", "admin"].includes(activeSection) ? "grid" : "hidden"} gap-5 xl:grid-cols-[1.6fr_1fr]`}>
        <div id="liga-pronosticos" ref={predictionsTopRef} className={activeSection === "pronosticos" ? "space-y-3" : "hidden"}>
          ...
          <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs dark:border-slate-800 dark:bg-slate-900/60"><div className="flex items-center justify-between gap-3"><button type="button" onClick={() => setMatchFiltersOpen((current) => !current)} className={`flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl px-3 font-black sm:flex-none ${matchFiltersOpen || hasMatchFilters ? "bg-blue-700 text-white" : "border border-slate-200 bg-white text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}>{matchFiltersOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />} Filtros {hasMatchFilters && <span className="rounded-full bg-white/20 px-1.5 py-0.5 text-[9px]">activos</span>}</button><span className="shrink-0 text-[10px] font-semibold text-slate-500 sm:text-[11px]">{filteredMatches.length}/{matches.length} partidos</span></div><div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 sm:hidden" aria-label="Acceso rápido por fecha"><button type="button" onClick={() => setRoundFilter("Todas")} className={`min-h-10 shrink-0 rounded-full px-3 text-[10px] font-black ${roundFilter === "Todas" ? "bg-blue-700 text-white" : "border border-slate-200 bg-white text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"}`}>Todas</button>{footballDates.map((stage) => <button type="button" key={stage} onClick={() => setRoundFilter(stage)} className={`min-h-10 shrink-0 rounded-full px-3 text-[10px] font-black ${roundFilter === stage ? "bg-blue-700 text-white" : "border border-slate-200 bg-white text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"}`}>{stage}</button>)}</div>{matchFiltersOpen && <div className="grid gap-3 sm:grid-cols-3"><label className="hidden font-bold text-slate-600 sm:block dark:text-slate-300">Fecha futbolera<select value={roundFilter} onChange={(event) => setRoundFilter(event.target.value)} className="mt-1 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm dark:border-slate-700 dark:bg-slate-800"><option value="Todas">Todas las fechas</option>{footballDates.map((stage) => { const match = matches.find((candidate) => candidate.stage === stage); return <option key={stage} value={stage}>{stage}{match ? ` · ${new Date(match.date).toLocaleDateString("es-CO", { timeZone: "America/Bogota", day: "numeric", month: "short" })}` : ""}</option>; })}</select></label><label className="font-bold text-slate-600 dark:text-slate-300">Estado<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)} className="mt-1 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-base sm:text-sm dark:border-slate-700 dark:bg-slate-800"><option value="all">Ver todos</option><option value="pending">Abiertos</option><option value="finished">Finalizados</option></select></label><label className="font-bold text-slate-600 dark:text-slate-300">Buscar rival o estadio<input type="search" value={matchSearch} onChange={(event) => setMatchSearch(event.target.value)} placeholder="Bucaramanga, El Campín..." className="mt-1 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-base sm:text-sm dark:border-slate-700 dark:bg-slate-800" /></label></div>}<div className="flex items-center justify-between gap-3"><span className="hidden text-[11px] text-slate-500 sm:inline">Filtra por jornada oficial, estado o rival.</span>{hasMatchFilters && <button type="button" onClick={() => { setRoundFilter("Todas"); setStatusFilter("all"); setMatchSearch(""); }} className="ml-auto min-h-10 rounded-xl bg-slate-200 px-3 font-black text-slate-700 dark:bg-slate-800 dark:text-slate-200">Limpiar filtros</button>}</div></div>
